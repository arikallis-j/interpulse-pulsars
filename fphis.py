...
M = 1.9 # M в массах Солнца
Ir = 20 # I_r в массах Солнца * км^2
R = 12*10**5 # радиус в см
 
epsGR = 3*M/R*100000
sRc = 1 - 1/2*epsGR # ОТО - коррекция радиуса кривизны
sB0 = 1 + 3/4*epsGR # ОТО - усиление магнитного поля
sPs = (1 - Ir*3/(R/100000)**3*epsGR)/(1 - epsGR) # GR rho_GJ
sR0 = 1 - 3/8*epsGR # GR R_cap/R_cap st
s = (Ir/50)**0.5/(R/1.2E6)**3 # параметр - пересчет магнитного полч через Pdot
...
def B_bgi(P, P_dot, chi, b_args):
	epsilon = b_args['epsilon']
	const = exp10(12) * epsilon**(-1/2)
	p = P/exp10(0)
	p_dot = P_dot/exp10(-15)
	return const * s* sB0 * p_dot**(0.7)
def B_mhd(P, P_dot, chi, b_args):
	const = exp10(12)
	p = P/exp10(0)
	p_dot = P_dot/exp10(-15)
	return const * s * sB0 * p**0.5 * p_dot**0.5 / np.sqrt(1 + sin(chi)**2)

def R_0(P, f_s):
	omega = Omega(P)
	return sR0 * f_s**(1/2) * (omega*R_st/c_lt)**(1/2) * R_st

def R_c(r_m):
	try:
		return 4/3 * sRc * R_st**2 / r_m
...
def H_RS(P, B, R_c, theta_b):
	...
	r_c = R_c/exp10(7)
	b = B/exp10(12) 
	return const * sPs**(-3/7) * cos_t**(-3/7) * p**(3/7) * r_c**(2/7) * b**(-4/7)

def pho_GJ(Omega, B, theta_b):
	return sPs*Omega*B*cos(theta_b)/(2*PI*c_lt)
